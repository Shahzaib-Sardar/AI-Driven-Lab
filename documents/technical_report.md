...
The current prompt is stored in `backend/prompts/monthly_spending_summary_v1.txt`. It tells the model to:
- stay concise,
- avoid invented numbers,
- return valid JSON,
- include optional retrieved context when available,
...
The backend includes a lightweight RAG-inspired retrieval layer that uses local knowledge documents and a JSON-backed store to inject grounding snippets into the prompt. A full production-grade vector DB such as Chroma or FAISS is not currently used, which keeps the MVP simpler while still supporting optional retrieved context.
...
### Current Status
The current implementation uses computed backend context and a lightweight retrieval service. The backend builds a local RAG store from knowledge docs, retrieves top snippets for the prompt, and injects them before calling OpenAI. The backend also preserves retrieved metadata, although frontend citation display is not yet exposed.
...
| RAG evaluation report file | Exists | `tests/evaluation/outputs/report.md` is generated by `backend/scripts/evaluate_rag.py`, though the current report contains no prompt results until prompts are added to the evaluation set |
